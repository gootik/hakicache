...
-spec compile(cache_key(), cache_value()) -> ok | {error, list(), list()}.
compile(Key, Val) ->
...
get_function(Val) when is_list(Val) andalso length(Val) > 1000 ->
...
mod_name(Key) ->
    list_to_atom("estatic_" ++ atom_to_list(Key)).